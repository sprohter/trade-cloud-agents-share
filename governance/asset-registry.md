...
| 位置 | 状态 | 说明 |
|------|------|------|
| `https://github.com/<owner>/<private-agents-backup>` | ✅ 个人私有备份镜像 | `.agents/` 可版本化内容的 GitHub Private 脱敏导出，用于个人备份和 Windows / macOS 新设备 bootstrap；仅本人可见，不向同事或外部朋友分享；不是真实运行态真源，不保存 `runtime/local-secrets/`、真实 MCP 配置、`runtime/state/`、`runtime/tmp/`、`collab/sessions/`、`archive/`、`case-studies/`、`upstream/`、`claude-code/lessons.md` |
...
| `.agents/scripts/sync/sync-github-agent-repos.ps1` | ✅ 活跃 | GitHub 双仓统一入口：编排 `sync-github-safe-mirror.ps1` 和 `sync-github-share-architecture.ps1`，写入双仓运行摘要；不改变各自安全边界；内部 Git 调用默认清空继承 credential helper 并在 Windows 使用 Git Credential Manager |
| `.agents/scripts/sync/sync-github-safe-mirror.ps1` | ✅ 活跃 | GitHub Private 安全镜像一键同步脚本：默认增量比较 changed paths、默认每周日自动全量、保留 7 天兜底、支持 `-ForceFull`、Windows 默认 `http.sslBackend=openssl` 与 `credential.helper=manager`、fast-forward push、远端校验；默认运行态目录在 `<user-home>/.codex/automations/daily-agents-safe-mirror-sync/` |
...
| 位置 | 状态 | 说明 |
|------|------|------|
| `https://github.com/<owner>/<private-agents-backup>-share` | ✅ 活跃 | 面向同事和外部朋友的安全架构分享仓库；保留有助于理解的项目背景、工具名和流程名，发布前替换个人身份、本机路径、内网 IP、真实系统 URL、公司私有域名、服务器日志路径和私人备份库 URL；不从个人备份库复制历史 |
| `.agents/scripts/sync/sync-github-share-architecture.ps1` | ✅ 活跃 | 分享版导出脚本：白名单复制、阻断扫描、fresh clone、overlay、fast-forward 推送校验；内部 Git 调用默认清空继承 credential helper 并在 Windows 使用 Git Credential Manager；默认运行态目录在 `<user-home>/.codex/automations/agents-architecture-share-sync/` |

治理口径：

- 分享方案见 `governance/architecture.md` 的“GitHub 公开分享架构仓库方案”。
- 个人备份库和分享仓库是两条线：备份库只给本人；分享仓库只承载纯架构和通用工具思路。
- 分享仓库禁止出现账号、密码、token、cookie、私钥、连接串、个人身份、本机路径、内网 IP、真实系统 URL、公司私有域名、服务拓扑、截图、附件、业务数据、事故现场和运行态配置。
...
| `skills/*/` | ✅ 唯一权威来源 | 原从 `.kiro/skills/` 全量同步（2026-04-14）；2026-04-16 起 `.kiro/skills/` 已移入 `_cleanup/`，`.agents/skills/` 为唯一权威，当前 26 个 skill 目录 |
| `skills/zentao-ops-cli/capsule.yml` | ✅ pilot | 高频禅道 CLI skill 的结构化路由摘要试点；不替代 `SKILL.md`，不参与自动路由 |
| `skills/ep-log-search/capsule.yml` | ✅ pilot | applog 原始日志检索 skill 的结构化路由摘要试点；不记录凭证、真实服务器目标或真实日志内容 |
| `skills/sql-diagnosis/capsule.yml` | ✅ pilot | SQL 诊断与数据验证 skill 的结构化路由摘要试点；默认只读，写操作仍需人工确认和影响评估 |
...
| `knowledge/infrastructure/apifox-contract-source.md` | ✅ 活跃 | Apifox 接口契约定位、MCP/skill 关系、本地 OpenAPI 缓存兜底、公司账号 token 私有配置边界 |
...
| `governance/router-core-evolution.md` | ✅ 活跃 | 结构化路由内核演进方案：承接同事 SOP Router Core 的 graph / policy / capsule / eval / M1-M4 memory 思想，明确渐进吸收路线和暂不切换边界 |
...
| `governance/sedimentation.md` | ✅ 活跃 | 沉淀触发、分类、M1-M4 记忆分层、来源等级、写入规则 |
| `governance/self-correction.md` | ✅ 活跃 | 路由纠偏、证据标准、角色边界、Codex 权限受限误判纠偏 |
...
| `codex/runtime-configs.md` | ✅ 活跃 | Codex 运行时配置资产索引；记录 Codex 权限模式、`config.toml` 与外部 CLI/API 访问受限判断口径 |
...
| `templates/document-template.md` | ✅ 活跃 | 通用文档模板（已迁入 `.agents/templates/`）；新增文档需标注 M1-M4 记忆层或说明不适用 |
| `templates/shared-skill-template-SKILL.md` | ✅ 活跃 | 共享 skill 模板（已迁入 `.agents/templates/`） |
| `templates/skill-capsule.template.yml` | ✅ 活跃 | skill 轻量 capsule 模板：承接 SOP Router Core 的低风险思想，包含 M3 memory_layer 字段，仅作为未来结构化路由评估模板，不改变现有 skill 加载方式 |
| `templates/route-eval-case.template.yml` | ✅ 活跃 | 路由回归用例模板：记录典型用户请求、期望路由、风险策略和禁选路径，用于后续路由校验 |
...
| `tools/apifox-docs.md` | ✅ 活跃 | Apifox 接口契约与 UI 元数据读取工具索引；MCP / official API / 本地缓存兜底 |
...
| `scripts/apifox/apifox-contract.ps1` / `scripts/apifox/apifox-contract.js` | ✅ 试运行 | Apifox OpenAPI 本地缓存只读读取器；PowerShell 包装 + Node 大 JSON 核心，输出脱敏接口契约摘要、source path、mtime、project id 与 stale warning |
| `scripts/apifox/apifox-meta.js` | ✅ 试运行 | Apifox 项目/环境/目录树元数据只读读取器；official API 使用 Authorization header，本地目录树只读 UI tree cache |
| `scripts/apifox/run-apifox-mcp.js` | ✅ 试运行 | Apifox 官方 MCP server 启动包装；从本机私有 `MCP_APIFOX__APIFOX_ACCESS_TOKEN` 映射到官方 `APIFOX_ACCESS_TOKEN`，不把 token 写入 MCP JSON |
...
| `case-studies/zentao-bug-edit-richtext-preservation-20260515.md` | ✅ 活跃 | 禅道 Bug 编辑正文保全复盘：局部 REST PUT 清空 `steps` 的恢复路径、工具修复和防再发清单 |
...
*最后更新：2026-05-16（补充 M1-M4 记忆分层与 capsule pilot 试点）*
