...
**使用场景**：`trade.ps1 verify mcp-paths` 或完整性检查提示 `test-db` / `prod-db-primary` / `prod-db-secondary` 仍指向旧 `.kiro/settings/mcp-servers/mysql-server/index.js`。
...
- 只处理 `test-db`、`prod-db-primary`、`prod-db-secondary` 三个业务 MySQL MCP server 的 `args` 行。
